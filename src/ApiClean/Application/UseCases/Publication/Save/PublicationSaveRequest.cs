...
namespace Application.UseCases.Publication.Save
{
    public class PublicationSaveRequest
    {
        ...
        public PublicationSaveRequest(Domain.User.User autor, string title, string content, DateTime dateCreated, List<Comment> comment, Domain.Topic.Topic topic)
        {
            Publication = new Domain.Publication.Publication(Guid.NewGuid(), autor, title, content, dateCreated, comment, topic);

        }

        public PublicationSaveRequest(Guid id, Domain.User.User autor, string title, string content, DateTime dateCreated, List<Comment> comment, Domain.Topic.Topic topic)
        {
